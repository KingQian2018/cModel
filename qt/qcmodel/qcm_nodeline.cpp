...
void QCM_PreNodeLine::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    if (node2() == nullptr || node1()->collidesWithItem(node2())) //判断图形项是否存在相交
        return;
    ...
    QPointF p1 = QPointF(node2()->pos().x(), node2()->pos().y()); // item的左上方点  + item在场景的位置点（偏移得到左上点在场景中的位置）
    QPointF p2 = QPointF(node1()->pos().x(), node1()->pos().y());
