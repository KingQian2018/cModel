...
#define lua_c
// #define lua_writestring qDebug
// #define lua_writeline() qDebug("\n")
...
int qcm_main(int argc, char **argv)
{
  int status, result;
...
void QCM_Lua::run()
{
  qDebug("start lua");
  int status, result;
  status = lua_pcall(m_L, 2, 1, 0); /* do the call */
  result = lua_toboolean(m_L, -1);  /* get result */
  report(m_L, status);
  qDebug("finish lua");
}
